...
function WorkerDetail({ workerList, setWorkerList, workerDetail }) {
  ...
  const { photo, name, age, phoneNumber, email, address, active } =
    workerDetail;

  function deleteWorkerFromList() {
    let newWorkerList = workerList.filter((eachWorker) => {
      return !(workerDetail.name === eachWorker.name);
    });
          ...
          {!update && (
            <div className="name-age-number-email-adress-state">
              ...
              <div>
                <b>Worker state:</b>
                <br></br>
                {/* RECUERDA: === true o === false, esta muy feo */}
                    ...
                    onChange={(event) => {
                      ...
                      workerListTemplate.forEach((worker, position) => {
                        if (worker.name === workerDetail.name) {
                          workerListTemplate[position].name =
                            event.target.value;
                        }
                      });
                    ...
                    onChange={(event) => {
                      ...
                      workerListTemplate.forEach((worker, position) => {
                        if (worker.phoneNumber === workerDetail.phoneNumber) {
                          workerListTemplate[position].phoneNumber =
                            event.target.value;
                        }
                      });
                    ...
                    onChange={(event) => {
                      ...
                      workerListTemplate.forEach((worker, position) => {
                        if (worker.email === workerDetail.email) {
                          workerListTemplate[position].email =
                            event.target.value;
                        }
                      });
                    ...
                    onChange={(event) => {
                      ...
                      workerListTemplate.forEach((worker, position) => {
                        if (worker.address === workerDetail.address) {
                          workerListTemplate[position].address =
                            event.target.value;
                        }
                      });
